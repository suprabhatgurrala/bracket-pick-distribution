...
def get_bracket_data(scoring_period_id=None, year=2026, women=False):
    slug = f"tournament-challenge-bracket-women-{year}" if women else f"tournament-challenge-bracket-{year}"
    base_url = f"https://gambit-api.fantasy.espn.com/apis/v1/challenges/{slug}/"
    params = {}
...
@st.cache_data
def fetch_bracket_data(year, women=False):
    results = []

    # Get initial metadata to find all scoring periods
    data = get_bracket_data(year=year, women=women)
    ...
    for i, scoring_period_id in enumerate(period_ids):
        data = get_bracket_data(scoring_period_id, year=year, women=women)
...
def main():
    ...
    col_title, col_bracket, col_year = st.columns([3, 1, 1])
    with col_bracket:
        bracket = st.selectbox("Select Bracket", options=["Men's", "Women's"], index=0)
    with col_year:
        year = st.selectbox("Select Year", options=reversed(years), index=0)

    women = bracket == "Women's"

    with col_title:
        st.title(f"🏀 ESPN {bracket} Bracket Pick Distribution {year}")

    with st.spinner(f"Fetching latest bracket data from ESPN for {year}..."):
        espn_df = fetch_bracket_data(year, women)
